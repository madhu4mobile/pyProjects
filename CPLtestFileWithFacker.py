...
def generate_payenm():
    payee_name_1 = fake.first_name()
    payee_name_2 = fake.last_name()
...
def generate_pyeadd():
    payee_address_line_1 = fake.street_address()
    payee_address_line_2 = fake.secondary_address()
...
def generate_addpye():
    payee_address_line_3 = fake.building_number()
    payee_address_line_4 = fake.postcode()
    return f"ADDPYE,{payee_address_line_3},{payee_address_line_4},"


# Function to generate random record for PYEPOS
def generate_pyepos():
    payee_city = fake.city()
    payee_state = fake.state_abbr()
    payee_zip = fake.zipcode().split('-')[0]
    payee_country = 'USA'
...
def main():
    num_records = 5  # Number of sets of records to generate

    with open("output.txt", "w") as file:
        file.write(f"FILHDR,PWS,CDSCHECKS6,{generate_random_date()},{generate_random_time()},\n")

        for _ in range(num_records):
            records = [generate_pmthdr, generate_payenm, generate_pyeadd, generate_addpye, generate_pyepos,
                       generate_rmtdtl]
            for record_type in records:
                record = record_type()
                file.write(record + "\n")

        file.write(f"FILTRL,{6 * num_records},")
